...
exports.getAll = async (req, res) => {
    try {
        ...
        const [rows] = await connection.query(
            `SELECT m.id, u.nombre, u.email, m.numero_colegiado, m.especialidad_id, e.nombre as especialidad_nombre,
                    m.valor_consulta, m.activo
             FROM medicos m
...
exports.create = async (req, res) => {
    try {
        const { nombre, email, numero_colegiado, especialidad_id, valor_consulta, contrasena } = req.body;
        if (!nombre || !email || !numero_colegiado || !especialidad_id || !contrasena) {
            return res.status(400).json({ success: false, message: 'nombre, email, numero_colegiado, especialidad_id, and contrasena are required' });
        }
        const connection = await pool.getConnection();
        ...
        const [userResult] = await connection.query(
            'INSERT INTO usuarios (nombre, email, contrasena, rol_id, activo) VALUES (?, ?, ?, 3, 1)',
            [nombre, email, hashedPassword]
        );
        const [medicoResult] = await connection.query(
            'INSERT INTO medicos (usuario_id, numero_colegiado, especialidad_id, valor_consulta, activo) VALUES (?, ?, ?, ?, 1)',
            [userResult.insertId, numero_colegiado, especialidad_id, valor_consulta || 0]
        );
        connection.release();
        res.status(201).json({ success: true, data: { id: medicoResult.insertId, usuario_id: userResult.insertId, nombre, email, numero_colegiado, especialidad_id, valor_consulta: valor_consulta || 0, activo: 1 }, message: 'Médico created successfully' });
    } catch (error) {
        console.error(error);
...
exports.update = async (req, res) => {
    try {
        const { id } = req.params;
        const { nombre, email, numero_colegiado, especialidad_id, valor_consulta } = req.body;
        const connection = await pool.getConnection();
        ...
        await connection.query('UPDATE medicos SET numero_colegiado = ?, especialidad_id = ?, valor_consulta = ? WHERE id = ?', [numero_colegiado, especialidad_id, valor_consulta, id]);
        connection.release();
        res.status(200).json({ success: true, data: { id: parseInt(id), nombre, email, numero_colegiado, especialidad_id, valor_consulta, activo: 1 }, message: 'Médico updated successfully' });
    } catch (error) {
        console.error(error);
